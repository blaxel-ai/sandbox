...
// A timed keepAlive process adopted after a restart must still be bounded by
// its timeout: the timer StartProcessWithName armed died with the previous
// run, so adoption has to re-arm it for whatever of the timeout is left.
func TestAdoptedKeepAliveProcessEnforcesTimeout(t *testing.T) {
	originalLogDir := ProcessLogDir
	dir := t.TempDir()
	ProcessLogDir = dir
	t.Cleanup(func() { ProcessLogDir = originalLogDir })
	...
	// A 1-second timeout that already expired before the restart: the process
	// must be killed right after adoption. PID mirrors the OS pid, as
	// StartProcessWithName sets it.
	...
	for time.Now().Before(deadline) {
		...
		content, readErr := os.ReadFile(scaleFile)
		if proc.Status == StatusKilled && readErr == nil && string(content) == "-" {
			return
		}
		time.Sleep(100 * time.Millisecond)
	}
	proc, _ := pm.GetProcessByIdentifier("survivor")
	content, _ := os.ReadFile(scaleFile)
	t.Fatalf("expected the adopted process to be killed by its expired timeout and the hold released, got status %q scale %q", proc.Status, content)
}
