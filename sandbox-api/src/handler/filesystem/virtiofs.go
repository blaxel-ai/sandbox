...
func ConfigureGitForVirtioFS() {
	...
	// Write a system-level gitconfig that applies to all users.
	// This sets core.supportsAtomicFileCreation=false which causes git
	// to use rename() instead of link()+unlink() for atomic file operations,
	// avoiding the virtiofs nlink bug entirely.
	gitConfig := []byte(
		"# Auto-generated by sandbox-api to work around virtiofs hard link bug (ENG-1990)\n" +
			"# See: https://linear.app/blaxel/issue/ENG-1990\n" +
			"[core]\n" +
			"\tsupportsAtomicFileCreation = false\n",
	)

	// Try /etc/gitconfig (system-level, requires root or write access)
	if err := os.MkdirAll("/etc", 0755); err == nil {
		if err := os.WriteFile("/etc/gitconfig", gitConfig, 0644); err != nil {
			logrus.WithError(err).Debug("[virtiofs] failed to write /etc/gitconfig")
			// Fall back to user-level config
			if home := os.Getenv("HOME"); home != "" {
				configPath := filepath.Join(home, ".gitconfig")
				if _, err := os.Stat(configPath); os.IsNotExist(err) {
					if writeErr := os.WriteFile(configPath, gitConfig, 0644); writeErr != nil {
						logrus.WithError(writeErr).Warn("[virtiofs] failed to write ~/.gitconfig")
					} else {
						logrus.Info("[virtiofs] wrote ~/.gitconfig with virtiofs workaround")
					}
				}
			}
		} else {
			logrus.Info("[virtiofs] wrote /etc/gitconfig with virtiofs workaround")
		}
	}
}
